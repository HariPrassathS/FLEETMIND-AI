...
export default function DispatcherFleetPage() {
  ...
  const handleCreateVehicle = (e: React.FormEvent) => {
    ...
    fleetMindStore.createLorry({
      ...
      image_url: newVehicleForm.image_url || undefined,
  ...
  const handleUpdateVehicleCapacity = (e: React.FormEvent) => {
    e.preventDefault();
    if (!editLorry) return;
    fleetMindStore.updateLorry(editLorry.id, {
      max_weight_kg: Number(editLorry.max_weight_kg),
      max_volume_m3: Number(editLorry.max_volume_m3),
      fuel_efficiency_km_per_l: Number(editLorry.fuel_efficiency_km_per_l),
      model: editLorry.model,
      image_url: editLorry.image_url,
    });
    setEditLorry(null);
  };
  ...
  return (
    <>
      ...
      <main className="p-4 sm:p-8 space-y-6 max-w-7xl mx-auto w-full">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-lg w-full p-6 space-y-5 animate-in fade-in my-auto">
              ...
              <form onSubmit={handleCreateVehicle} className="space-y-4 text-xs">
                ...
                <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 space-y-2.5">
                  ...
                  <div className="flex items-center gap-3">
                    <VehicleAvatar
                      src={newVehicleForm.image_url}
                      lorryCode={newVehicleForm.lorry_code}
                      model={newVehicleForm.model}
                      size="lg"
                    />
                      ...
                      <div className="flex flex-wrap gap-1">
                        {VEHICLE_PRESET_IMAGES.map((preset) => (
                          <button
                            key={preset.id}
                            type="button"
                            onClick={() => setNewVehicleForm({ ...newVehicleForm, image_url: preset.url })}
                            className={`px-2 py-0.5 rounded-lg text-[9px] font-bold border transition ${
                              newVehicleForm.image_url === preset.url
        ...
        {/* Modal: Edit Vehicle & Profile DP */}
        {editLorry && (
          <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
            <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-lg w-full p-6 space-y-5 animate-in fade-in my-auto">
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <h3 className="text-base font-black text-slate-900">Edit Vehicle & DP: {editLorry.lorry_code}</h3>
              ...
              <form onSubmit={handleUpdateVehicleCapacity} className="space-y-4 text-xs">
                ...
                <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 space-y-2.5">
                  ...
                  <div className="flex items-center gap-3">
                    <VehicleAvatar
                      src={editLorry.image_url}
                      lorryCode={editLorry.lorry_code}
                      model={editLorry.model}
                      size="lg"
                    />
                    ...
                    <div className="flex-1 space-y-2">
                      <input
                        type="url"
                        placeholder="Paste vehicle image URL..."
                        value={editLorry.image_url || ''}
                        onChange={(e) => setEditLorry({ ...editLorry, image_url: e.target.value })}
                        className="w-full px-3 py-1.5 rounded-xl border border-slate-200 text-xs font-medium"
                      />
                      <div className="flex flex-wrap gap-1">
                        {VEHICLE_PRESET_IMAGES.map((preset) => (
                          <button
                            key={preset.id}
                            type="button"
                            onClick={() => setEditLorry({ ...editLorry, image_url: preset.url })}
                            className={`px-2 py-0.5 rounded-lg text-[9px] font-bold border transition ${
                              editLorry.image_url === preset.url
                                ? 'bg-blue-600 text-white border-blue-600'
                                : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-100'
                            }`}
                          >
                ...
                <div>
                  ...
                  <input
                    type="text"
                    required
                    value={editLorry.model}
                    onChange={(e) => setEditLorry({ ...editLorry, model: e.target.value })}
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 font-bold"
                  />
                </div>
